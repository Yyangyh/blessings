		...
		<view class="order">
			<view class="order_num" v-for="(item,index) in data_list" :key='item.id' >
				...
				<view class="order_handle" v-if="data.status == 2 || data.status == 3|| data.status == 4">
					...
					<text v-if="data.refundstate == null"  @click="jump('./s_order_refund?oid='+ data.id)">{{data.status == 4?'申请售后':'退款/退货'}}</text>
					<block v-else>
						...
						<text v-else-if="data.refundstate == 4"  @click="jump('./s_order_refund?oid='+ data.id)">已拒绝</text>
						<text v-else-if="data.refundstate == 5"  @click="jump('./s_order_refund?oid='+ data.id)">已取消</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			determine(){//确定收货
				...
				uni.showModal({
				    ...
				    success: function (res) {
				        if (res.confirm) {
				            that.service.entire(that,'get',that.APIconfig.api_root.subuser.threeuser.g_Collect,{
								id:that.data.id,
			...
			payment() {
				...
				uni.showModal({
				    ...
				    success: function (res) {
				        if (res.confirm) {
				            that.service.entire(that, 'get', that.APIconfig.api_root.subhome.threehome.g_pay, {
				            	user_id:that.$store.state.user.id,
				            	orderid: that.data.id,
				            	payment_id: that.payment_id,
				            }, function(self, ref) {
				            	self.service.order(ref,self,'../assemble_order','pages/subuser/assemble_order')
				            })
				        } else if (res.cancel) {
				           return
				        }
		...
		onLoad(options) {
			...
			this.service.entire(this, 'get', this.APIconfig.api_root.subuser.threeuser.g_OrderDetail, {
				orderid: options.id,
